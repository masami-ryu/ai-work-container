...
tools: Read, Grep, Glob, Bash, WebFetch, mcp__context7, mcp__msdocs, mcp__github-mcp-server, mcp__serena
model: sonnet
---
...
**Serena MCP失敗時**:
1. フォールバック: `Read`ツールでファイル全体を読み取り
2. `Grep`ツールで依存関係を追跡
3. レビュー続行（Phase 2の完了条件を調整）

**msdocs/context7 MCP失敗時**:
1. フォールバック: 一般的なベストプラクティスを適用
2. エビデンスなしの指摘として記録
3. レビュー続行

**GitHub MCP失敗時**:
1. フォールバック: `gh` CLIコマンドでPR情報を取得
   ```bash
   gh pr view [PR番号] --json number,title,body,files,additions,deletions
   gh pr diff [PR番号]
   ```
2. 取得できない場合はレビュー中断、エラーを報告
