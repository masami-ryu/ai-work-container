...
このスキルはレビュー対象の入力を受け取り、形式を判定して必要な情報を取得し、構造化データとして返却します。code-reviewerエージェントから呼び出され、レビュー実施に必要な差分情報や変更ファイル一覧を準備します。
...
取得した情報を統一フォーマットに変換してテキスト形式で返却。エージェントが読み取り可能な明確な構造で出力。
...
- **1.0.0** (2025-12-25): 初版リリース（code-reviewerエージェントから入力判定・情報取得処理を分離）
