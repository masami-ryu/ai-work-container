"""スレッド返信ハンドラ: スレッド返信による質問回答 + 割り込み指示"""
...
from ..slack_messages import ask_resolved_blocks, interrupt_ack_blocks
...
def register_thread_reply_handler(
    ...
    async def on_message(event, say):
        ...
        # --- TASK-103: 割り込み指示処理 ---
        # pending 質問がない場合、スレッドに紐付いたセッションへの割り込み指示と判断
        session = session_manager.get_session_by_thread(thread_ts)
        if session is None:
            return
        ...
        # TASK-107: 割り込みキューに投入
        session_manager.enqueue_interrupt(session, text)

        # TASK-105: 確認メッセージをスレッドに投稿
        try:
            await bot.post_message(
                blocks=interrupt_ack_blocks(text),
                text=f"Interrupt: {text[:80]}",
                thread_ts=thread_ts,
            )
        except Exception:
            logger.warning("Failed to post interrupt ack for session %s", session.session_id)

        # TASK-106: 監査ログ記録
        audit.record(
            correlation_id=session.session_id,
            request_type="interrupt",
            tool_name=None,
            summary=text[:200],
            decision="accepted",
            responder_user_id=user_id,
