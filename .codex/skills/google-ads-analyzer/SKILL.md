...
description: |
  ...
  P-MAX のサイトリンク削除、YouTubeリンク確認、非モバイル抑制、デバイス設定変更など、Google Ads API を使った軽微な設定確認・変更依頼にも使用する。
...
- Google Ads API の Developer Token が環境に設定済みであること
- 秘密情報（Developer Token、OAuth client secret、Refresh Token）は出力・保存しないこと

## 認証・接続プリフライト

分析や設定変更の前に、次を確認する。

- `list_accessible_customers` が成功すること
- `GOOGLE_ADS_DEVELOPER_TOKEN` が未設定またはプレースホルダでないこと
- ADC（通常 `~/.config/gcloud/application_default_credentials.json`）が存在すること
- MCP ツールがセッションに露出していない場合は、`references/operations-recipes.md` の「MCP未露出時の直接実行」を使う
...
- 設定変更（mutate）が必要な場合は、先に **[references/query-recipes.md](references/query-recipes.md)** で対象 resource_name を取得し、変更手順は **[references/operations-recipes.md](references/operations-recipes.md)** を参照すること
...
**前回レポートを踏まえる場合**:
- 前回レポートに記載された対象期間を今回も再取得し、前回レポート値との差分を確認する
- 差分が出た場合は、ローカルアクション CV の後日追加・CV値設定変更・期間入れ替わりを分けて説明する
- 今回新たに追加された直近期間の評価では、前回期間の後日追加率を感度として使い、断定を避ける
...
ローカルアクション、来店、電話、Google Hosted 系 CV は後日追加されることがある。短期評価では次を必ず適用する。

| 条件 | 扱い | 表現 |
|---|---|---|
| 期間終了日が分析日から3日以内 | 未成熟 | 「暫定」「計測ラグ補正待ち」「要再取得」 |
| 14日以下の期間比較 | 要監視 | 「悪化確定」ではなく「弱く見える」「要監視シグナル」 |
| 直近日の CV 0 / CPA 高騰 | 判定保留 | 後日 CV 追加の可能性を明記 |
| 前回同種期間で CV が後日増加 | 感度分析必須 | 今回期間にも同程度の補正余地があると注記 |

未成熟または要監視の期間では、原則として `HIGH` 判定を付けない。例外は、費用規模が大きく、終了日から4日以上経過しても CPA/ROAS 悪化が残る場合に限る。
...
### Phase 4: 設定変更モード（明示依頼時のみ）

ユーザーが「削除して」「抑制して」「変更して」など、広告設定の変更を明示した場合のみ実行する。

**安全ルール**:

- 変更前に対象の `resource_name`、ステータス、URL、デバイス種別などを取得して特定する
- 変更対象は依頼範囲に限定する。関連アセット本体を削除せず、必要ならキャンペーンとの紐づけだけ外す
- mutate 後は必ず再取得し、変更後ステータスまたは値を確認する
- 最終回答に変更前後、対象 `resource_name`、確認結果、反映ラグの注意点を記載する
- 変更ログを残す必要がある場合は `ai/reviews/YYMMDD_HHmm_[概要].md` に保存する

**P-MAX 固有の注意**:

- サイトリンク削除は `campaign_asset` の remove を使い、`asset` 本体は削除しない
- デバイス条件は削除できない。非モバイル抑制は `campaign_criterion.bid_modifier = 0.0`、モバイル維持は `1.0` を使う
- YouTubeリンクのサイトリンク削除と、`YOUTUBE_VIDEO` アセット削除は別物として扱う
- P-MAX設定変更後は反映・学習にラグがあるため、3〜7日後の再確認を推奨する
...
## 2.5 データ成熟度
期間終了日 / 分析日との差 / 成熟度（確定寄り・要監視・未成熟） / 判断保留の有無
...
**分析の観点**:
- CPA 前月比 50% 以上悪化 → HIGH で要因調査。ただし短期データ成熟度ゲートで未成熟または要監視なら MEDIUM 以下に下げ、再取得後に判定
- CTR 1% 未満 → 広告文・アセットの訴求力不足
- モバイル CV 構成比 90% 超 → ローカルビジネス特性として記録
- P-MAX 変更後 2 週間未満 → 学習期間として評価を保留
- ローカルアクション CV は 1〜3 日以上の計測ラグがあり、直近の CV 0 や CPA 高騰は暫定値として注記
- 前回レポートの同一期間を再取得して CV が増えている場合、今回の直近期間も同程度に補正される可能性を感度分析として記載
